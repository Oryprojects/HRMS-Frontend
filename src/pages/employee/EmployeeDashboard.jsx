...
const EmployeeDashboard = () => {
	...
	const handleRecallLeave = (leaveId) => {
		if (window.confirm('Are you sure you want to recall this leave request?')) {
			console.log('Recalling leave:', leaveId);
	...
	return (
		<div className="flex min-h-screen bg-bg-slate font-brand text-brand-blue">
			...
			<main className="flex-1 flex flex-col">
						...
						<div className="flex items-center gap-6">
							...
							<div>
								...
								<p className="text-[10px] text-brand-blue/40 uppercase font-black tracking-[0.2em] mt-0.5">
									{user.designation || "Employee Dashboard"}
								</p>
							</div>
						</div>
				...
				{/* Content Area */}
				<div className={`flex-1 ${activeTab === 'profile' ? 'p-2 md:p-6' : 'p-4 md:p-8'} flex flex-col ${activeTab === 'profile' ? 'gap-2' : 'gap-8'}`}>
					{activeTab === 'dashboard' && (
						<>
							{error && (
