  ...
  const handlePhotoUpload = async (event) => {
    ...
    if (file) {
      ...
      try {
        ...
        localStorage.setItem("user", JSON.stringify({ ...storedUser, photoPath }));
      } catch (err) {
        alert(err.message);
      }
    }
    ...
    if (editing) {
      try {
        ...
        localStorage.setItem("user", JSON.stringify({
          ...
          photoPath: updated.photoPath || storedUser.photoPath,
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="space-y-2">
                            ...
                            <div className="grid grid-cols-3 gap-2">
                              ...
                              <input name="middleName" value={form.middleName || ''} onChange={handleChange} disabled={!editing} className={`w-full bg-gray-50 border-none rounded-xl px-4 py-3 text-sm font-bold text-brand-blue focus:ring-2 focus:ring-brand-yellow/50 transition-all ${!editing ? 'cursor-not-allowed opacity-80' : ''}`} placeholder="Middle Name" />
                              ...
                              <div key={field.name} className="space-y-2">
                                ...
                                {isDropdown ? (
                                  <select
                                    ...
                                    disabled={isDisabled}
                                    className={`w-full bg-gray-50 border-none rounded-xl px-4 py-3 text-sm font-bold text-brand-blue focus:ring-2 focus:ring-brand-yellow/50 transition-all ${isDisabled ? 'cursor-not-allowed opacity-80' : ''}`}
                                  >
